...
def test_append_included_institutions_outside_of_page(
    add_institution, add_citation):

    """
    When ids are passed for institutions that fall outside of the default page,
    merge the extra facets onto the bottom of the list.
    """
    ...
    facets = institution_facets(depth=2, include=[i3.id, i4.id])
...
def test_dedupe_institutions_inside_of_page(add_institution, add_citation):

    """
    When ids are passed for institutions that are already present in the
    default page, don't duplicate the include list.
    """
    ...
    facets = institution_facets(include=[i2.id, i3.id])
